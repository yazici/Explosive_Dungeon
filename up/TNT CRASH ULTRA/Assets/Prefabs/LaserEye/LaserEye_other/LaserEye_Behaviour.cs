...
public class LaserEye_Behaviour : MonoBehaviour {
    ...
    {
        if (isExplosion) { if (LaserEye_Trigger.isPlayerOnTriger() && !MoveAndJump.Invisible || LaserEYETRIGGER2.isPlayerOnTriger() && !MoveAndJump.Invisible) { MoveAndJump.Died = true; print("Player has been killed by script " + this.name); } }
    ...
    public IEnumerator EnemySpawn()
    {
        ...
        if (isExplosion) { if (LaserEye_Trigger.isPlayerOnTriger() && !MoveAndJump.Invisible  || LaserEYETRIGGER2.isPlayerOnTriger() && !MoveAndJump.Invisible) { MoveAndJump.Died = true;} }
        explosion_wav.Play();
        isExplosion = true;
